...
public class IdeaCipher {
    ...
    private void setKey(String charKey) {
        setKey(IdeaUtils.makeKey(charKey, KEY_SIZE));
    }
    ...
    public void crypt(byte[] data, int offset) {
        ...
        int x1 = IdeaUtils.concat2Bytes(data[offset], data[offset + 1]);
        int x2 = IdeaUtils.concat2Bytes(data[offset + 2], data[offset + 3]);
        int x3 = IdeaUtils.concat2Bytes(data[offset + 4], data[offset + 5]);
        int x4 = IdeaUtils.concat2Bytes(data[offset + 6], data[offset + 7]);
        // Each round
    ...
    private static int[] generateSubkeys(byte[] userKey) {
        ...
        for (int i = 0; i < userKey.length / 2; i++) {
            key[i] = IdeaUtils.concat2Bytes(userKey[2 * i], userKey[2 * i + 1]);
        }
